...
function ListAvances() 
{
    ...
    useEffect(() => {
      ...
      function handleApproveAvance(userId) {
        Swal.fire({
          title: 'Are you sure?',
          text: 'Are you sure you want to approve the demande?',
          icon: 'warning',
          ...
          .then((result) => {
            if (result.isConfirmed) {
              // Send a request to the server to approve the demande with the given userId
              return fetch(`https://localhost:5001/approveAvance/${userId}`, {
                method: 'PUT',
                headers: {
          ...
          .then((response) => {
            if (response.ok) {
              Swal.fire('Approved!', 'The demande has been approved.', 'success');
      ...
      function handleRejectAvance(userId) {
        Swal.fire({
          title: 'Are you sure?',
          text: 'Are you sure you want to reject the demande?',
          icon: 'warning',
          showCancelButton: true,
          ...
          .then((result) => {
            if (result.isConfirmed) {
              // Send a request to the server to reject the demande with the given userId
              return fetch(`https://localhost:5001/rejectAvance/${userId}`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                },
          ...
          .then((response) => {
            if (response.ok) {
              Swal.fire('Rejected!', 'The demande has been rejected.', 'success');
              setpendingDemande((prevUsers) =>
                prevUsers.filter((user) => user.id !== userId)
              );
              window.location.reload(); // Refresh the page to fetch the updated user list
...
<li data-username="form elements advance componant validation masking wizard picker select" className="nav-item">
            ...
            <li data-username="Table bootstrap datatable footable" className="nav-item">
              <a href="/ListAvances" className="nav-link "><span className="pcoded-micon"><i className="feather icon-server" /></span><span className="pcoded-mtext">Advance requests List </span></a>
            </li>
    ...
    <div className="col">
      <div className="card">
        <div className="card-header">
          <h3>List of Advance Requests</h3>
        </div>
                  ...
                  <tr key={request.id} className="unread">
                    ...
                    <td className="text-center">
                      <div className="btn-group">
                        <button className="btn btn-danger" onClick={() => handleRejectAvance(request.id)}>Reject</button>
                        <button className="btn btn-success" onClick={() => handleApproveAvance(request.id)}>Approve</button>
                      </div>
                    </td>
                  </tr>
                ))}
